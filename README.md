...
* Trabajar la especificación, implementacuón y uso de un tipo abstracto de datos (TAD).
...
i. `nuevaAgenda`, la cual crea una agenda en la cual no se ha programado ninguna actividad
ii. `agregar.Actividad`, la cual permite incluir una nueva actividad en la planificación de actividades de la agenda;
iii. `eliminarActividad`, la cual permite cancelar una actividad programada previamente en la aenda;
iv. `listarActividades`, la cual lista todas las actividades programadas para un día dado, ordenadas segun su horario;
v. `buscarActividad`, la cual, dada una fecha-hora de una actividad, permite ver el nombre de la actividad programada;
vi. `incluirActividades`, la cual, dada otra agenda, incluye todas las actividades programadas en esa otra agenda como actividades de la agenda actual.

###Modelo de Representación
```
const MAX : int 
var nombre: String &rarr; String
var duración: String &rarr; int
var prioridad: String &rarr; int 
```

###Invariante de Representación 
El siguiente invariante se debe cumplir sobre su estructura de datos en todo momento. Debe verificar que se cumplir&aacute; luego de cada operaci&oacute;n. En caso contrario, debe imprimir un error, y la operaci&oacute;n no se har&aacute; efectiva. *El programa debe seguir en funcionamiento luego de producire el error.*
...
###Operaciones 
A continuaci&acute;n se presentan pre y post condiciones para cada operaci&oacute;n. Debe verificar que se cumple la precondici&oacute;n antes de realizar la operaci&oacute;n. En caso contrario, debe imprimir un error y no realizar la operaci&oacute;n. Las postcondiciones indican qu&eacute; debe realizar la operaci&oacute;n.
...
> { `Pre: ` &not;(&exist; *x* &isin; dom(*al.nombre*): choca(*a2*, *x*, *al.duracion*(*x*)))
>> &and; |dom(*al.nombre*)| + |dom(*a2.nombre*)| &le; *a1*.MAX } 
{ `Post:` *a1.nombre* = *a1*<sub>0</sub>.*nombre* &cup; *a2.nombre* 
>> &and; *al.duraci&oacute;* = *a1.duraci&oacute;n &cup; *a2.duraci&oacute;n 
>> &and; *a1.prioridad* = *al*<sub>0</sub>.*prioridad* &cup; *a2.prioridad* }
  &vellip;
`Fin TAD`

Esta especificación utiliza los predicados auxiliares:
i. duraci&oacute;nValida, que dado un `String` que representa una fecha-hora y un entero que representa una duración, se satisface si la duración permite que la actividad termine en el horario válido;
ii. fechaHoraV&aacute;lida, dado un `String`, se satisface si corresponde al formato de una fecha-hora válida;
iii. esFechaV&aacute;lida, se satisface si el entero corresponde a una fecha v&aacute;lida; iv. esHoraV&aacute;lida, se satisface si el entero corresponde a una hora valida
v. seSolapan, se satisface si las fechas-horas y duraciones son tales que se solapan, las fechas-horas son dados como St1ing y la duraciones como enteros
vi. choca, se satisface si, en la agenda dada como primer argumento, hay alguna actividad tal que se solape con la fecha-hora y duración dadas como segundo y tercer argumento.
...
* esFechaV&aacute;lida(*f*) = 1 s f mod 100:::; 30 1\ 1 s (f div 100) mod 100 s 12 1\ f div 10000 ~ 2007
* esHora Valida(*h*) = (6 &le; *h* div 100 &le; 21 &and; 0 &le; *h* mod 100 &le; 59) &or; (*h* div 100 = 22 &and; *h* mod 100 = 0)
...
i. agregarActividad;
ii. eliminarActividad;
iii. listarActividades;
iv. buscarActividad;
v. incluirActividades;
vi. listarActividadeslmportantes;
vii. cantidadActividades (de la agenda);
viii. cantidad Actividades (de la agenda. para una fecha dada);
ix. salir del sistema.
